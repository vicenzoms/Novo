...
def simular_politica_dual(s_star, s, S, params):
    """Executa a simulação horária interrompendo a impressora assim que o estoque sai do estado crítico."""
    ...
    for t in range(1, Horizonte_T + 1):
        ...
        if t == Tempo_Chegada_Impressao:
            ...
            # PARADA INTELIGENTE: Só continua imprimindo outra se O PEDIDO REGULAR AINDA NÃO CHEGOU e o estoque AINDA ESTIVER CRÍTICO (<= s*)
            if Ot > 0 and estoque_atual <= s_star:
                Jt = 1
        ...
        # Gatilho Emergencial (s*) - Inicia impressão de 1 peça APENAS se o pedido convencional já tiver sido feito (Ot > 0)
        estoque_atual = It + Pt - Bt
        if estoque_atual <= s_star and Jt == 0 and Ot > 0:
            Jt = 1
            Tempo_Chegada_Impressao = t + L_ef
            Custo_Total += C2
...
def simular_politica_dual_com_historico(s_star, s, S, params):
    """Gera o diário detalhado e histórico temporal com interrupção sob demanda."""
    Horizonte_T = params['Horizonte_T']
    N = params['N']
    L_rep = params['L_rep']
    L_ef = params['L_ef']
    ...
    for t in range(1, Horizonte_T + 1):
        ...
        if t == Tempo_Chegada_Impressao:
            ...
            if Ot > 0 and estoque_atual <= s_star:
                Jt = 1
                Tempo_Chegada_Impressao = t + L_ef
                evento_descricao.append("Estoque ainda crítico (<= s*); iniciando mais 1 peça 3D")
            else:
                Jt = 0
                Tempo_Chegada_Impressao = float('inf')
                evento_descricao.append("Estoque suficiente (> s*) ou Pedido Chegou; Impressora Parada")
