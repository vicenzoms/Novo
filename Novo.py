...
def simular_politica_dual(s_star, s, S, params):
    ...
    return Custo_Total, disponibilidade
...
def otimizar_gatilhos_grid(S, params):
    ...
    for s in range(0, limite_s):
        for s_star in range(0, s + 1):
            ...
            disps_parciais = []
            for _ in range(3):
                c, d = simular_politica_dual(s_star, s, S, params)
    ...
    return melhor_s_star, melhor_s, melhor_custo, melhor_disp
...
# --- MODO 3: OPTIMIZER MA (MANUFATURA ADITIVA) ---
    ...
    if botao_ma:
        with st.spinner("A gerar Cenários Estocásticos e Otimizar... Aguarde."):
            ...
            # 2. Executa o Grid Search Simulado
            melhor_s_star, melhor_s, melhor_custo_total, disponibilidade = otimizar_gatilhos_grid(S_teto, params)
